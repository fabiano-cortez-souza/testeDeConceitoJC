...
class JsonUtilsTest {
	...
	void validJsonStringShouldReturnValidJavaObject() {
	    ...
	    String msisdn = StringUtils.repeat("1", Constants.DEFAULT_MSISDN_LENGTH);
	    String type = "refil";
	    String amount = String.valueOf(Long.MAX_VALUE);
	    String transactionID = Utils.generateId();
		
		String validJsonString = new TransactionHistoryVO(timestamp,
														  msisdn,
														  type,
														  amount,
														  transactionID).toString();
		
		String jsonString = String.valueOf(JsonUtils.parseJsonStringToJavaObject(validJsonString, TransactionHistoryVO.class));
